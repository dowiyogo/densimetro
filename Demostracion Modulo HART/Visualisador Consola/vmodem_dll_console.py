...
frame = [bytes(b'\xff\xff\xff\xff\xff\x02\x80\x00\x00\x82'),bytes(b'\xff\xff\xff\xff\xff\x82\x80\x01\x00\x00\x02\x00\x00\x01'),bytes(b'\xFF\xFF\xFF\xFF\x0F\x02\x80\x00\x00\x82'), bytes(b'\xFF\xFF\xFF\xFF\xFF\x01\x80\x00\x00\x81'), bytes(b'\xFF\xFF\xFF\xFF\xFF\x22\x80\x00\x00\xA2'), bytes(b'\xff\xff\xff\xff\xff\x02\x81\x00\x00\x83'),bytes(b'\xff\xff\xff\xff\xff\x82\x80\x01\x00\x00\x00\x00\x00\x03'),bytes(b'\xff\xff\xff\xff\xff\x02\x80\x00\x03\x00\x82'),bytes(b'\xff\xff\xff\xff\xff\x02\x80\x00\x00\x8f')]
index_frame = 0
	...
	while True:
		...
		if len(buff_modem) > 0:
			...
			if buff_modem == b'r':
				...
				print("<= recv : [", len(buff_modem),"] ", buff_modem)##buff_modem[0] + buff_modem[1]*256)#
