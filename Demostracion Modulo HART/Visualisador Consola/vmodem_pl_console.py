...
#fffffffffffffffffffdacs123456789009876543211234567890123c
	...
	while True:
		...
		if len(buff_modem) > 0:
			...
			if buff_modem == b'r':
				...
				print("<= recv : [", len(buff_modem),"] ", buff_modem.decode("ascii"))#buff_modem[0] + buff_modem[1]*256)

			elif buff_modem == b't':
				buff_modem = paco.recv_wait()
				print("=> send : [", len(buff_modem),"] ", buff_modem.decode("ascii"))

			elif buff_modem == b'e':
				buff_modem = paco.recv_wait()
				print("<= error : [", len(buff_modem),"] ", buff_modem.decode("ascii"))
