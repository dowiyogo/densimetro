...
/******** LIBRARY PARAMETERS *******************/
// Arduino Serial
#define _UART_BAUD_RATE			1200							// For Arduino Serial
#define _UART_CONF				SERIAL_8O1						// For Arduino use
#define SERIALX					Serial3							// Used serial port

/******** APPLICATION LAYER PARAMETERS *********/
#define _UNIVERSAL_COMMAND_REVISION 	((char)0x06)			// Universal Command Revision version
...
const char _DEVICE_TYPE        = 0x01;					// Default device type
const char _DEVICE_UNIQUE_ID[] = {0x00,0x00,0x02};		// Default unique device id
const char _DEVICE_REVISION[3] = {0x00,0x00,0x00};		// Default 3 bytes device revision
const char _PHYSICAL_SIGNAL_CODE = 0x00;				// Common tables, Table 10. Bell202 current
const char _FLAG = 0x0A;								// Common tables, Table 11. Multi-Sensor Field Device
...
const unsigned char PREAMBLE_DEFAULT_SIZE  = PREAMBLE_MIN_SIZE;	// Default preabmle size
const          char PREAMBLE_DEFAULT_VALUE = 0xFF;				// !Established by protocol (it's a byte => char (not unsigned))
const unsigned char START_OF_MSG_SIZE      = 2;					// !Established by protocol for FSK and RS485
const unsigned char POLLING_ADDRESS_SIZE   = 1;					// !Established by protocol. Delimeter Byte
const unsigned char DELIMETER_SIZE         = 1;					// !Established by protocol. N° of bytes for Polling address
...
const unsigned char DATA_MAX_NBYTES        = 254;				// !Established by protocol. Max. number of bytes allowed in [data] per frame
const unsigned char DATA_ERROR_BYTES       = 1;					// !Error bytes for Slave response
...
enum HARTCommand{COMMAND0=0, COMMAND1=1, COMMAND6=6,
	COMMAND255=255};
																// HART commands (2 implemented) (Using COMMAND3 as message passer)
