...
#define DP_VERSION_BUGFIX           ((uint8_t)0x11)
...
uint16_t       i;
   ...
   #endif /* #if VPC3_SERIAL_MODE */
	//printf("oki\n");
	/*if (bError != DP_VPC3_ERROR){PORTB|=(1<<PB7);// for( i = 0x16; i < ASIC_RAM_LENGTH; ) 
		_delay_ms(3000);
		PORTB&=~(1<<PB7);
		_delay_ms(500);}*/
...
uint8_t       bCount;
   ...
   if( ( bCfgLength > 0 ) && ( bCfgLength <= CFG_BUFSIZE ) )
   {
	...
	if ( bTempInputDataLength > DIN_BUFSIZE ) {
	...
	if ( bTempOutputDataLength > DOUT_BUFSIZE) {
