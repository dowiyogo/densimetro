   ...
   if( DpAppl_TestApplEvent( eDpApplEv_IoOut ) )
   {
      ...
      if( pToOutputBuffer )
      {
         ...
         /** @todo Write the output data to the output modules. */
			PORTA=sDpAppl.abDpOutputData[0]%10;
			PORTC=(sDpAppl.abDpOutputData[0]%100)/10;	
			if((sDpAppl.abDpOutputData[0]/100)==1) PORTC|=(1<<PC4);
			else if((sDpAppl.abDpOutputData[0]/100)==2) PORTC|=((1<<PC4)|(1<<PC5));else PORTC&=~((1<<PC4)|(1<<PC5));
